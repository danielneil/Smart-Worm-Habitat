...
The CFT should also be able to transported easily e.g. in the back of pickup/utility.
...
* An engineering firm will be required to use the drafts (as above) to construct the CFT. 
...
The unit will be supported by legs which effectively raise the bed to allow for the collection of vermicompost underneath:
	...
	The legs will raise the bed 40cm off the ground (therefore the CFT have a height of legs(40cm) + frame height(75cm), 115cm).
...
The cutting bar should be "raised" 1 inch on top of the mesh, in order to maximise the volume of the harvest (or scrape). Maybe "guide bars" can run the length of the CFT, which the cutting bar "rests" on, with these guide bars, one inch in height, resting on the mesh, below.  
...
The prefab design will hopefully be allow for scalablity. 

*What are the limitations of the solution?*

Weight of the CFT could be significant (especially when full), making mobility troublsome - hence wheels on legs.
...
The entire project will be under an open source model, with all generated docuemntation stored on Github.
